...
\item \emph{Convergence (Section~\ref{sec:conv}).} We show that PRG with
      ...
      carried through the reflection step. An error at the base point enters
      through the projection inequality of the previous step; carrying it out we
      find the base-point error contributes $\nu_k \le C\varepsilon_{k-1}$ with a
      constant $C$ that does \emph{not} contain $1/\lambda$ (in the projection
      inequality the operator already appears multiplied by $\lambda$, so the
      cross term carries $\lambda$ as a factor, not its inverse). Consequently
      the perturbed convergence statement is an extension of \cite{Malitsky2015}
      and \cite{Censor2017}, with no new perturbation mechanism, and we do not
      base any novelty claim on it.
...
a_{k-1} \ge 0 .
\]
For the \emph{exact} projection case the argument is complete: combining
Lemmas~\ref{lem:onestep}--\ref{lem:tele} and adding
$\lambda L\norm{\bar x^{k+1}-r^k}^2 - 2\lambda a_k$ to both sides gives, using
the identity $c_2 - \lambda L = c_1$ with $c_1 = 1-\lambda L(1+\sqrt2)$ and
$c_2 = 1-\sqrt2\lambda L$,
\[
\varphi_{k+1} \le \varphi_k - c_1\norm{x^k-x^{k-1}}^2
- c_1\norm{\bar x^{k+1}-r^k}^2 + 2\lambda a_k ,
\]
and $a_k \le 0$ makes $\varphi_k$ nonincreasing. This part has been verified
line by line. The inexact projection $x^{k+1} = \bar x^{k+1} + e^k$ adds two
perturbations: one through the squares in $\varphi_{k+1}$, contributing
$O(\varepsilon_k)$ times unbounded norm factors; and one at the \emph{base
point} $x^k = \bar x^k + e^{k-1}$, entering through the previous projection
inequality and contributing $\nu_k \le C\varepsilon_{k-1}$. The first is handled
by a multiplicative quasi-Fej\'er reduction, $2\varepsilon_k\norm{\bar
x^{k+1}-z} \le \varepsilon_k(1+\norm{\bar x^{k+1}-z}^2) \le
\varepsilon_k(1+\varphi_{k+1})$, giving $\varphi_{k+1} \le (1+\alpha_k)\varphi_k
- c_1(\cdots) + 2\lambda a_k + \beta_k$ with $\sum_k \alpha_k, \sum_k \beta_k <
\infty$. Then Lemma~\ref{lem:qf} (multiplicative form) applies; since $a_k \le
0$, summing also gives $\sum_k (-a_k) < \infty$ and hence $a_k \to 0$,
...
\emph{Status.} The four lemmas, the exact-projection assembly (the identity
$c_2-\lambda L=c_1$ and the resulting nonincreasing $\varphi_k$), and the
base-point perturbation constant $\nu_k \le C\varepsilon_{k-1}$ (with $C$ free
of $1/\lambda$) are complete and have been checked line by line. What remains
for the full write-up is the precise statement of the multiplicative
quasi-Fej\'er lemma so that it does not absorb the negative term, and, in
infinite dimension, the weak limit step; in the finite-dimensional setting of
the experiments the latter is routine. We flag this rather than presenting the
theorem as fully established.
\end{proof}
...
and $7.2$--$14.7$ in time. All configurations return feasible iterates
(constraint ratio at most $1.0000$), whereas a fixed inner budget violates the
