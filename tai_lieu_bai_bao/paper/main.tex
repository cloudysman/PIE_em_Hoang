...
\item \emph{Convergence (Section~\ref{sec:conv}).} We show that PRG with
      ...
      through the projection inequality of the previous step, and tracking it
      through the reflection is the part that must be carried out from scratch;
      whether it carries an amplifying factor $1/\lambda$ or cancels to first
      order is left to the full write-up, since convergence only requires it to
      be summable.
...
\emph{Status.} The four lemmas and the exact-projection assembly (the identity
$c_2-\lambda L=c_1$ and the resulting nonincreasing $\varphi_k$) are complete
and have been checked line by line. What remains for the full write-up is the
explicit constant $C$ in $\nu_k$ (in particular whether it carries a factor
$1/\lambda$), the precise statement of the multiplicative quasi-Fej\'er lemma so
that it does not absorb the negative term, and, in infinite dimension, the weak
limit step. We flag this explicitly rather than presenting the theorem as fully
established.
\end{proof}
